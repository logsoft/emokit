...
class Emotiv(QtCore.QThread):
    ...
    def getlinuxdevices(self):
        ...
        for inputs in rawinputs:
            try:
                if os.path.exists(inputs[0] + "/manufacturer"):
    ...
    def run(self):
        while not self.exiting:
            ...
            if data != "":
                decrypted = self.cipher.decrypt(data[:16]) + self.cipher.decrypt(data[16:])
                self.lastpacket = EmotivPacket(decrypted, self.sensors)
